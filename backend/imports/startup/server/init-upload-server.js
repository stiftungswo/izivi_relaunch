...
export function processRequest(request, { uploadDir } = {}) {
  ...
  const form = formidable.IncomingForm({ // eslint-disable-line
  ...
  return new Promise((resolve, reject) => {
    form.parse(request, (error, { operations }, files) => {
      ...
      operations = JSON.parse(operations); // eslint-disable-line
...
WebApp.connectHandlers.use(apolloUploadExpress());
